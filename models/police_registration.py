...
class PoliceRegistration:
    ...
    def from_db_row(cls, row: Dict[str, Any]) -> "PoliceRegistration":
        ...
        return cls(
            ...
            reservation_id=UUID(row["reservation_id"]) if isinstance(row["reservation_id"], str) else row["reservation_id"],
    ...
    def __str__(self) -> str:
        ...
        return (f"PoliceRegistration(id={self.id}, status={self.status.value}, "
                f"reservation_id={self.reservation_id})")
    
    def __repr__(self) -> str:
        """Detailed string representation"""
        return (f"PoliceRegistration(id={self.id}, created_at={self.created_at}, "
...
def calculate_success_rate(registrations: list[PoliceRegistration]) -> float:
    """
    Calculate success rate for a list of registrations
    
    Args:
        registrations: List of PoliceRegistration instances
    ...
    successful_count = sum(1 for registration in registrations if registration.is_successful())
    return (successful_count / len(registrations)) * 100


def calculate_booking_success_rate(registrations: list[PoliceRegistration]) -> float:
    """
    Calculate booking success rate for a list of registrations
    
    Args:
        registrations: List of PoliceRegistration instances
        
    Returns:
        Booking success rate as percentage (0-100)
    """
    if not registrations:
        return 0.0
    
    successful_count = sum(1 for registration in registrations if registration.status_booking.is_success())
    return (successful_count / len(registrations)) * 100


def calculate_checkout_success_rate(registrations: list[PoliceRegistration]) -> float:
    """
    Calculate checkout success rate for a list of registrations
    
    Args:
        registrations: List of PoliceRegistration instances
        
    Returns:
        Checkout success rate as percentage (0-100)
    """
    if not registrations:
        return 0.0
    
    successful_count = sum(1 for registration in registrations if registration.status_check_out.is_success())
    return (successful_count / len(registrations)) * 100


def group_by_status(registrations: list[PoliceRegistration]) -> Dict[RegistrationStatus, list[PoliceRegistration]]:
    """
    Group registrations by overall status
    
    Args:
        registrations: List of PoliceRegistration instances
        
    Returns:
        Dictionary mapping status to list of registrations
    """
    groups = {}
    for registration in registrations:
        if registration.status not in groups:
            groups[registration.status] = []
        groups[registration.status].append(registration)
    return groups


def group_by_booking_status(registrations: list[PoliceRegistration]) -> Dict[BookingStatus, list[PoliceRegistration]]:
    """
    Group registrations by booking status
    
    Args:
        registrations: List of PoliceRegistration instances
        
    Returns:
        Dictionary mapping booking status to list of registrations
    """
    groups = {}
    for registration in registrations:
        if registration.status_booking not in groups:
            groups[registration.status_booking] = []
        groups[registration.status_booking].append(registration)
    return groups


def group_by_checkout_status(registrations: list[PoliceRegistration]) -> Dict[CheckoutStatus, list[PoliceRegistration]]:
    """
    Group registrations by checkout status
    
    Args:
        registrations: List of PoliceRegistration instances
        
    Returns:
        Dictionary mapping checkout status to list of registrations
    """
    groups = {}
    for registration in registrations:
        if registration.status_check_out not in groups:
            groups[registration.status_check_out] = []
        groups[registration.status_check_out].append(registration)
    return groups


def get_status_success_rates(registrations: list[PoliceRegistration]) -> Dict[str, float]:
    """
    Calculate success rates for different phases
    
    Args:
        registrations: List of PoliceRegistration instances
        
    Returns:
        Dictionary mapping phase to success rate percentage
    """
    return {
        "overall": calculate_success_rate(registrations),
        "booking": calculate_booking_success_rate(registrations),
        "checkout": calculate_checkout_success_rate(registrations),
    }


def analyze_registration_timeline(registrations: list[PoliceRegistration]) -> Dict[str, Any]:
    """
    Analyze registration timeline patterns
    ...
    """
    current_count = sum(1 for reg in registrations if reg.is_current_registration())
    past_count = sum(1 for reg in registrations if reg.is_past_registration())
    future_count = sum(1 for reg in registrations if reg.is_future_registration())
    no_dates_count = len(registrations) - current_count - past_count - future_count
    
    durations = [reg.get_duration_days() for reg in registrations if reg.get_duration_days() is not None]
...
def get_issue_summary(registrations: list[PoliceRegistration]) -> Dict[str, int]:
    """
    Get summary of issues across all registrations
    ...
    """
    booking_issues = sum(1 for reg in registrations if reg.has_booking_issues())
    checkout_issues = sum(1 for reg in registrations if reg.has_checkout_issues())
    room_change_issues = sum(1 for reg in registrations if reg.has_room_change_issues())
    overall_failures = sum(1 for reg in registrations if reg.is_failed())
    
    return {
        "booking_issues": booking_issues,
        "checkout_issues": checkout_issues,
        "room_change_issues": room_change_issues,
